class Sanitizer {
    ...
    unapply = function (func) {
        let { apply, construct } = typeof Reflect !== 'undefined' && Reflect;
        return (thisArg, ...args) => apply(func, thisArg, args);
    }

    lookupGetter = function(object, prop) {
        while (object !== null) {
            const desc = Object.getOwnPropertyDescriptor(object, prop);

            if (desc) {
                if (desc.get) {
                    return this.unapply(desc.get);
                }

                if (typeof desc.value === 'function') {
                    return this.unapply(desc.value);
                }
            }
    ...
    removeNode = (currentNode) => {
        const getNextSibling = this.lookupGetter(Element.prototype, 'nextSibling');
